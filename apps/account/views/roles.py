...
class SystemRoleManager(APIView):
    """
        系统角色管理 机房管理
    """
    def get(self, request):
        ...
        if queryset and page_size:
            ...
            serializer = serializers.SystemRolesManagerSerializer(instance=page_roles, many=True)
            return pg.get_paginated_response(serializer.data)

        elif queryset:
            serializer = serializers.SystemRolesManagerSerializer(instance=queryset, many=True)
            data = {
    ...
    def post(self, request):
        verify_data = serializers.SystemRolesManagerSerializer(data=request.data)
    ...
    def patch(self, request):
        ...
        print(pk)
        if pk:
            queryset = models.Role.objects.filter(id=pk).first()
            serializer = serializers.SystemRolesManagerSerializer(queryset, data=request.data, partial=True)
