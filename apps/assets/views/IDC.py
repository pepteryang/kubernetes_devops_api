...
class AssetsIDCInfo(APIView):
    ...
    def get(self, request):
        ...
        if queryset:
            ...
            serializer = serializers.AssetsIDCManagementSerializer(instance=page_roles, many=True)
    ...
    def post(self, request):
        verify_data = serializers.AssetsIDCManagementSerializer(data=request.data)
    ...
    def patch(self, request):
        ...
        if pk:
            queryset = models.IDC.objects.filter(id=pk).first()
            serializer = serializers.AssetsIDCManagementSerializer(queryset, data=request.data, partial=True)
    ...
    def delete(self, request):
        data = request.data
        try:
            models.IDC.objects.filter(id=data.get('uuid')).delete()
            data = {
                "status": 200,
                "code": 66666,
                "message": "数据删除成功"
