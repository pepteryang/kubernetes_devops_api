...
class ResourcesCategoriesInfo(APIView):
    """
        系统角色管理 机房管理
    """
    def get(self, request):
        ...
        if queryset and page_size:
            ...
            serializer = serializers.ResourcesCategoriesInfoSerializer(instance=page_roles, many=True)
            return pg.get_paginated_response(serializer.data)
        elif pk:
            queryset = models.KubernetesResourcesCategoriesInfo.objects.filter(id=pk)
            print(queryset)
            serializer = serializers.ResourcesCategoriesInfoSerializer(instance=queryset, many=True)
            data = {
    ...
    def post(self, request):
        verify_data = serializers.ResourcesCategoriesInfoSerializer(data=request.data)
    ...
    def patch(self, request):
        ...
        if pk and password:
            queryset = models.KubernetesResourcesCategoriesInfo.objects.filter(id=pk).first()
            serializer = serializers.ResourcesCategoriesInfoSerializer(queryset, data=request.data, partial=True)
    ...
    def delete(self, request):
        ...
        print(pk)
        try:
            ss = models.KubernetesResourcesCategoriesInfo.objects.get(id=request.data.get('uuid')).delete()
            print(ss)
            data = {
                "status": 200,
                "code": 66666,
